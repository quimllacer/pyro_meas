...
def analyze(df, points_p_period = 10, freq = 0.01, window = 51 ):
    ...
    for i in range(len(A)):
        ...
        if margin <= i < len(A)-margin:
            ...
            frequency = 0.01
            T_amp = 1
            R_L = 10e6 # This resistance has to be placed in paralel with the pyroelectric film.
            p_coeff = (np.sin(math.radians(phase))*current_amplitude) / (electrode_area* 2*np.pi*frequency*T_amp*R_L)
